        ...
        <el-col :span="24">
          ...
          >
            ...
            <el-table-column label="类型">
              <template #default="scope">
                <el-tag>{{ aptypeData.dicDescs(scope.row.type) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="项目">
              <template #default="scope">
                <el-tag>{{ apitemData.dicDescs(scope.row.item) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="方式">
              <template #default="scope">
                <el-tag>{{ apmodeData.dicDescs(scope.row.mode) }}</el-tag>
              </template>
            </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        item: [{ required: true, message: "请选择 项目", trigger: "blur" }],
        mode: [{ required: true, message: "请选择 方式", trigger: "blur" }],
        amount: [
          {
            required: true,
            message: "请输入 金额，没有写0",
  ...
  methods: {
    ...
    getdic() {
      ...
      // })
      this.aptypeData = this.dicstore.getDic("aptype");
      this.apitemData = this.dicstore.getDic("apitem");
      this.apmodeData = this.dicstore.getDic("apmode");
    },
    listMain() {
