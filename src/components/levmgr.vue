      ...
      >
        ...
        >
          ...
          <el-table-column prop="type" label="类型" width="100">
            <template #default="scope">
              <span>{{
                levitemData.dicDescs(scope.row.type, "itemid", "itemname")
              }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="pay" label="偿付方式" width="80">
            <template #default="scope">
              <span>{{
                payData.dicDescs(scope.row.pay, "itemid", "itemname")
              }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    batchshift() {
      ...
      AX("post", `levlist/batch/${this.statusid}`, uptdata).then((res1) => {
        if (res1) {
          //console.log(res1);
    ...
    activeActivity(isactive, idx, row) {
      ...
      AX("patch", `levlist/${row.id}`, { isactive }).then((res1) => {
        if (res1) {
          this.listMain();
        }
      });
    ...
    getdic(type = "hr") {
      ...
      this.search.fromdate = this.userstore.getperiod("kq").sdate;
      this.search.todate = this.userstore.getperiod("kq").edate;

      this.levitemData = this.dicstore.getDic("levitem");
    ...
    listMain() {
      ...
      block.status = this.search.status;
      block.pay = this.search.pay;
      block.type = this.search.type;
