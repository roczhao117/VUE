<template>
  <div>
    ...
    <div style="display: flex; margin-right: 4px">
      ...
      <el-tooltip content="增加培训计划" placement="top">
      ...
      <span style="margin-left: 4px">
        <el-date-picker
          ...
          placeholder="输入培训开始日期"
        ></el-date-picker>
        <el-date-picker
          size="small"
          style="width: 110px; margin-left: 8px"
          v-model="searchform.todate"
          placeholder="输入培训结束日期"
        ></el-date-picker>
      </span>
    ...
    <div style="display: flex; margin-top: 10px; height: 650px">
      ...
      >
        <el-table
          :data="tableData"
          style="width: 930px; font-size: 12px"
          v-loading="loading"
        ...
        >
          ...
          <el-table-column prop="item" label="类型" width="80">
            <template #default="scope">
              <span>{{ dictrtypeData.dicDescs(scope.row.type) }}</span>
            </template>
          </el-table-column>
            ...
            <template #default="scope">
              <span class="butgrp">
                <el-tooltip content="修改" placement="top">
                  ...
                  ></el-button
                ></el-tooltip>
                <el-tooltip content="学员" placement="top">
                  <el-button
                    size="small"
                    icon="user"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        item: [{ required: true, message: "请选择课程", trigger: "blur" }],
        status: [{ required: true, message: "请选择状态", trigger: "blur" }],
        type: [{ required: true, message: "请选择类型", trigger: "blur" }],
        step: [{ required: true, message: "请输入阶段", trigger: "blur" }],
        teachers: [{ required: true, message: "请输入老师", trigger: "blur" }],
        fromdate: [
          {
            required: true,
            message: "请输入日期",
        ...
        time: [{ required: true, message: "请输入开始时间", trigger: "blur" }],
  ...
  methods: {
    ...
    addStaff() {
      ...
      AX("get", `hrinfo/trstaff/${this.form.planid}`)
        .then((res) => {
    ...
    savenewstaff() {
      if (this.addvalue.length > 0) {
        ...
        for (let i = 0; i < this.addvalue.length; i++) {
          ...
          staff.score = 75;
          staff.status = "4"; //4 在timestatus中表示的是正常出勤
          uptdata.push(staff);
        }

        AX("post", "trstaff/addstaff", uptdata).then((res1) => {
          if (res1) {
    ...
    saveForm() {
      this.$refs.refform.validate((valid) => {
        if (valid) {
          if (this.neworedit) {
            AX("post", "trplan", this.form).then((res1) => {
              if (res1) {
            ...
            AX("patch", `trplan/${this.form.id}`, this.form).then((res1) => {
              if (res1) {
                this.dialogAddVisible = false;
                this.neworedit = false;
                this.listMain();
    ...
    activeActivity(isactive, idx, row) {
      ...
      AX("patch", `trplan/${row.id}`, { isactive }).then((res1) => {
        if (res1) {
          this.listMain();
        }
      });
    ...
    editstaffstatus(row) {
      this.$refs.refstaffform.validate((valid) => {
        if (valid) {
          AX("patch", `trstaff/${this.staffform.id}`, this.staffform).then(
            (res1) => {
              if (res1) {
    ...
    staffDelete(idx, row) {
      this.$confirm("此操作将删除该人的培训记录, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          AX("delete", "trstaff/" + row.id).then((res) => {
            if (res) {
              this.listTrstaff();
            }
          });
    ...
    listTrstaff() {
      ...
      AX(
        "get",
        `trstaff/bypid/${this.form.planid}?page=${this.staffpage.cpg}&pagesize=${this.staffpage.limit}`
      )
        .then((res) => {
