    ...
    <div>
        ...
        <div style="display:flex;margin:1px 1px 10px;">
            ...
            <span style="margin-left:20px;">
                <el-select v-model="prinfo.prgrp" placeholder="请选择参考周期" style="width:100%">
                    <el-option
        ...
        <div style="display:flex;margin:1px;margin-right:20px;">
            <el-tooltip content="与上月薪资周期数据对比" placement="top">
                <el-button type="primary" icon="el-icon-plus" @click="compared(1)">对比上期</el-button>
            </el-tooltip>
            <el-tooltip content="与去年同期数据对比" placement="top">
                <el-button type="primary" icon="el-icon-plus" @click="compared(2)">对比去年同期</el-button>
            </el-tooltip>
            <el-tooltip content="自定义周期对比" placement="top">
                <el-button type="primary" icon="el-icon-plus" @click="compared(3)">自定义周期对比</el-button>
            </el-tooltip>
        ...
        <div style="display:flex;margin-top:10px;height:650px">
            ...
            <div style="margin-left:20px;width:100%;min-width:800px;">
                ...
                >
                    ...
                    <el-table-column prop="comptype" fixed="left" label="对比类型" width="80">
                        <template #default="scope">
                    ...
                    <el-table-column prop="calperiod" label="停薪周期" width="100">
                        <template #default="scope">
                            <span v-if="scope.row.if_stop == '1'">{{ scope.row.calperiod }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="h.currsalary" label="当前工资" width="100"></el-table-column>>
                    <el-table-column
                    ...
                    <el-table-column prop="source_period" label="对比月份" width="120"></el-table-column>
                    <el-table-column prop="target_period" label="参考月份" width="120"></el-table-column>
            ...
            <el-dialog title="薪酬信息" width="1000px" v-model="dialogFormVisible">
                ...
                <template #footer>
                    <span class="dialog-footer">
                        ...
                        <el-button type="success" @click="calculateforsysid">试 算</el-button>
                        <el-button type="primary" @click="savepritemvalForm">确 定</el-button>
                    </span>
                </template>
            </el-dialog>
        </div>
        <!-- 人事新人表单 -->
        <div>
            <el-dialog title="自定义周期对比" width="630px" v-model="dialogComparedCustomVisible">
                <el-switch
                ...
                <template #footer>
                    <span class="dialog-footer">
                        <el-button @click="dialogComparedCustomVisible = false">取 消</el-button>
...
export default {
    ...
    data() {
        return {
            ...
            prtypes: [{ 'code': '0', 'itemname': '发薪员工' }, { 'code': '1', 'itemname': '停薪员工' },],
            prtype: '0',
    ...
    methods: {
        ...
        cusRules() {
            ...
            for (let ele of this.pritemData) {
                ...
                const moneyreg = /(^[1-9]([0-9]+)?(\.[0-9]{1,4})?$)|(^(0){1}$)|(^(0){1}\.[0-9]+$)|(^[0-9]\.[0-9]([0-9])?$)/;
        ...
        cmd_tohist() {
            this.$confirm('此操作将把当前薪资转化为历史数据，并清零。 是否继续?', '重要信息', {
        ...
        */
        async cmd_calculate(sysid = '-1') {
            ...
            AX('post', '/calculate', { 'accountgrp': this.prinfo.prgrp, 'sysid': sysid }).then(() => {
        ...
        handleDelete(idx, row, if_stop) {
            ...
            this.$confirm('此操作将' + if_stop_mesg + '该人当月的薪资, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                AX('patch', '/prstaff/' + row.id, { 'if_stop': if_stop }).then((res) => {
                    if (res) {
                        this.listMain();
                    }
                })
        ...
        getdic(type = 'hr') {
            ...
            if (type == 'hr') {
                ...
                AX('get', '/dicm/prsum1').then((res) => {
                    this.dicprsum1 = res.data;
                })
                    .catch(e => console.log(e.message))
                ...
                AX('post', '/pritemlst', { 'prgrp': this.prinfo.prgrp }).then((res) => {
                ...
                AX('get', '/dicm/pritemgcalgrp').then((res) => {
                    ...
                    this.prinfo.prgrp = res.data[0].itemid;
        ...
        listMain() {
            ...
            AX('get', '/prcompare/' + this.page.limit + '/' + this.page.cpg + '/' + block).then(res => {
