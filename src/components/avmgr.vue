<template>
  <div>
    ...
    <div style="display: flex; margin-right: 4px">
      <span style="margin-left: 4px; width: 120px">
        <el-select
          v-model="isstopkq"
      ...
      <el-date-picker
        ...
        size="small"
      ></el-date-picker>
      <el-tooltip content="根据开始结束日期刷新积假数据" placement="top">
        <el-button
          icon="Refresh"
          size="small"
          @click="refresh()"
      ...
      <el-tooltip content="积假设置" placement="top">
        <el-button
          icon="setting"
          size="small"
          @click="cmd_setting"
      ...
      >
        ...
        >
          ...
          <el-table-column
            ...
            width="80"
          ></el-table-column>
          <el-table-column prop="fromdate" width="100" label="开始日期">
          </el-table-column>
          <el-table-column prop="todate" width="100" label="结束日期">
          </el-table-column>
          <el-table-column prop="oth" width="80" label="加班(H)">
          </el-table-column>
          <el-table-column prop="levh" width="80" label="请假(H)">
          </el-table-column>

          <el-table-column prop="diffh" width="80" label="积假(H)">
          </el-table-column>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            <el-col :span="24">
              <el-form-item label="加班类型" label-width="80" size="small">
                <el-checkbox-group v-model="form.ottype" class="chkgrp">
                  <el-checkbox
                    v-for="item in dicottypeData"
                    :key="item.itemid"
                    :value="item.itemid"
                    :label="item.itemid"
                    :checked="item.chk1 == 1"
            ...
            <el-col :span="24">
              <el-form-item label="请假类型" label-width="80" size="small">
                <el-checkbox-group v-model="form.levtype" class="chkgrp">
                  <el-checkbox
                    v-for="item in diclevtypeData"
                    :key="item.itemid"
                    :value="item.itemid"
                    :label="item.itemid"
                    :checked="item.chk1 == 1"
          ...
          >
            <el-col :span="24">
              <el-form-item label="加班偿付" label-width="80" size="small">
                <el-checkbox-group v-model="form.otpay" class="chkgrp">
                  <el-checkbox
                    v-for="item in dicotpayData"
                    :key="item.itemid"
                    :value="item.itemid"
                    :label="item.itemid"
                    :checked="item.chk1 == 1"
            ...
            <el-col :span="24">
              <el-form-item label="请假偿付" label-width="80" size="small">
          ...
          >
            <el-col :span="24">
              <el-form-item label="加班状态" label-width="80" size="small">
                <el-checkbox-group v-model="form.otstatus" class="chkgrp">
                  <el-checkbox
            ...
            <el-col :span="24">
              <el-form-item label="请假状态" label-width="80" size="small">
...
import { mealsType, avOptions } from "../utils/comdata";
import {
  useDeptPosStore,
  useDicStore,
  useUserStore,
...
export default {
  ...
  methods: {
    ...
    saveForm(istemplate) {
      ...
      const A = {};
      A.id = "ottype";
      A.data = this.form.ottype;

      const B = {};
      B.id = "levitem";
      B.data = this.form.levtype;
      //pay================================================================
      const C = {};
      C.id = "otpay";
      C.data = this.form.otpay;

      const D = {};
      D.id = "levpay";
      D.data = this.form.levpay;

      //status================================================================
      const E = {};
      E.id = "docstatus";
      E.data = this.form.otstatus;

      const F = {};
      F.id = "docstatus";
      F.key = "chk2";
      F.data = this.form.levstatus;
      ...
      AX("post", "dic/avset", updatedata).then((res1) => {
        if (res1) {
          //console.log(res1);
    ...
    refresh() {
      ...
      AX("post", `avlist/refresh`, uptdata).then((res1) => {
        if (res1) {
          //console.log(res1);
    ...
    batchshift() {
      ...
      for (let i = 0; i < this.multipleSelection.value.length; i++) {
        ...
        uptdata.push(this.multipleSelection.value[i].id);
      }

      if (uptdata.length <= 0) {
        this.$message.error("请勾选需要处理的人员！");
        return;
      }
      if (!this.statusid) {
      ...
      //console.log("dddddddddddddddddddddddd", this.statusid);

      AX("post", `otlist/batch/${this.statusid}`, uptdata).then((res1) => {
        if (res1) {
          //console.log(res1);
    ...
    getdic(type = "hr") {
      ...
      this.dicottypeData = this.dicstore.getDic("ottype");
      this.diclevtypeData = this.dicstore.getDic("levitem");
